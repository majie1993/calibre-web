    ...
    def search(
        ...
        if self.active:
            ...
            if not search_results:
                log.warning("No search results found in Amazon response")
                return []
            
            try:
                links_list = [next(filter(lambda i: "digital-text" in i["href"], x.findAll("a")))["href"] for x in search_results]
            except (StopIteration, AttributeError, KeyError) as e:
                log.warning("Failed to extract links from Amazon search results: %s", e)
                return []
            
            if not links_list:
                log.warning("No digital-text links found in Amazon search results")
                return []
